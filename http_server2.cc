...
int main(int argc,char *argv[])
{
  ...
  int rc,i;
  fd_set readlist;	//temp for select
  fd_set connections;	//master file 
  int maxfd;
  int yes=1;

FD_ZERO(&connections);    // clear the master and temp sets
FD_ZERO(&readlist);
      ...
      /* for the accept socket, add accepted connection to connections */
		if(FD_ISSET(i, &readlist)){
			if (i == sock){
			  ...
			  if ((sock2 = accept(sock, (struct sockaddr *)&sa2,
						(socklen_t *)&addrlen)) == -1) { 
					perror("accept");
				} else {
					FD_SET(sock2, &connections); // add to master set
					if (sock2 > maxfd) {    // keep track of the maximum
						maxfd = sock2;
					}
